...
@section('content')
    <div class="container">
        ...
        @if ($test_report)
            ...
            @if ($test_report->status == 'pending')
                <form id="test-report-form" method="POST" action="{{ route('trc.sign', $test_report->id) }}">
                    @csrf

                    <div class="form-group mb-3">
                        <label for="signature">Contractor Signature</label>
                    ...
                    <button type="submit" class="btn btn-success mb-3">Sign Test Report</button>
                    @if ($test_report->status !== 'rejected')
                        <form method="POST" action="{{ route('trc.reject', $test_report->id) }}">
                            @csrf
                            <button type="submit" class="btn btn-danger mb-3">Reject</button>
                        </form>
                    @endif
                </form>
            @elseif ($test_report->status == 'signed')
                <div class="d-flex flex-column">
            ...
            @if ($test_report->status !== 'pending')
                <form method="POST" action="{{ route('trc.pending', $test_report->id) }}">
                    @csrf
                    <button type="submit" class="btn btn-warning mb-3">Pending</button>
                </form>
            @endif
        @else
        ...
        <a href="{{ route('trc.index') }}" class="btn btn-secondary">Back</a>
    </div>
